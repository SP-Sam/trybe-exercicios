...
const sendMarsTemperature = (onSucces) => {
  const currentTemperature = getMarsTemperature();
  setTimeout(() => {
    onSucces(currentTemperature);
  }, messageDelay());
};
...
const sendMarsTemperature = (onSucces, onFail) => {
  const sendMessage = Math.random() <= 0.6;

  const currentTemperature = getMarsTemperature();

  setTimeout(() => {
    if (sendMessage) onSucces(currentTemperature)
